...
通用 AI 编码行为规范放在 docs/ai-coding/standards/。
...
- 调整 `standards/llm-coding-guidelines.md` 中的通用 AI 编码行为规范
...
skill 会先读取 `docs/ai-coding/standards/` 下的通用规范，再加载当前项目中被选中的 `docs/ai-coding/<context-scope>/`，并按照这个上下文的编码风格和规则推进功能开发。如果同一个仓库里有多个 context，它会先确认应该使用哪一个。
